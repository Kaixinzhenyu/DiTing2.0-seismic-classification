...
from data_loader import dataloader,two_class_dataloader
...
def train(train_loader, model, criterion, optimizer, epoch, writer,reconstructed=False):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if reconstructed:
            reconstruction_loss = F.mse_loss(model.reconstruct(target), input.view(-1, 8640))#3*36*72=7776
...
def main():
    ...
    #模型选择 (cnn1d,resnet1d,vgg1d,alexnet1d,cnn2d,googlenet,alexnet,resnet18,vit,cct,vit_small,swin,mult_cnn,vgg11,
    ...
    # 设置TensorBoard的日志目录
    log_dir = '/root/autodl-tmp/Python projects/my_project/logs/'+'{}_{}_{}_{}_{}_{}_{}_{}'.format(dataset_id,dataset_choose,model_choose,Batch_size,learning_rate,Weight_decay,epochs,Num_classes)
    
    writer = SummaryWriter(log_dir)
    if Num_classes ==3:
        #数据集加载
        if dataset_choose=='1d':
            train_loader,val_loader =dataloader(natural_train_path_0=base_path+'/tensor dataset/natural_train_dataset.pt',
                                            natural_train_path_1=None,
                                            ep_train_path=base_path+'/tensor dataset/ep_train_dataset.pt',
                                            ss_train_path=base_path+'/tensor dataset/ss_train_dataset.pt',
        ...
        if dataset_choose=='1d':
            train_loader,val_loader =two_class_dataloader(natural_train_path_0=base_path+'/tensor dataset/natural_train_dataset.pt',
                                            natural_train_path_1=None,
                                            ep_train_path=base_path+'/tensor dataset/ep_train_dataset.pt',
                                            ss_train_path=base_path+'/tensor dataset/ss_train_dataset.pt',
                                            batch_size=Batch_size,validation_percentage=Validation_percentage)
            ...
            train_loader,val_loader =two_class_dataloader(natural_train_path_0=base_path+'/processed_data/'+dataset_choose+'/natural_train_STFT.pt',
                                            ...
                                            ss_train_path=base_path+'/processed_data/'+dataset_choose+'/ss_train_STFT.pt',
                                            batch_size=Batch_size,validation_percentage=Validation_percentage)
        elif dataset_choose=='MFCC_39_24' or dataset_choose == 'MFCC_36_72' or dataset_choose == 'MFCC_40_1001' or dataset_choose == 'MFCC_40_601':
    ...
    for epoch in range(epochs):
        ...
        }, is_best,dataset_choose,model_choose,Batch_size,learning_rate,Weight_decay,checkpoint_path,Num_classes)
